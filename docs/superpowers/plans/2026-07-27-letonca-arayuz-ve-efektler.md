...
- [ ] **Step 1: Ses efektlerini doğrula**

Sekiz ses dosyası `ios/Karavan/Resources/Sounds/` altında **zaten mevcut** — Kenney Interface Sounds (CC0) paketinden seçilip mono 44.1 kHz 16-bit WAV'a dönüştürüldü. Hangi sesin nereden geldiği `ios/Karavan/Resources/Sounds/KAYNAK.md` içinde yazılı. İndirmen veya üretmen gereken bir şey yok.

Varlıklarını doğrula:

```bash
ls ios/Karavan/Resources/Sounds/*.wav | wc -l
```

Beklenen: `8`. Dosya adları `lv-correct.wav`, `lv-wrong.wav`, `lv-complete.wav`, `lv-xp.wav`, `lv-heart.wav`, `lv-streak.wav`, `lv-tap.wav`, `lv-combo.wav`.
